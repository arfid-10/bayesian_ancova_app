                  ...
                  accordion(
                    ...
                    accordion_panel(
                      ...
                      selectInput(inputId = "plotCI",
                                  label = "Equal-tailed intervals or HDIs for 
...
server <- function(input, output, session) {
  ...
  output$brm <- DT::renderDT({
    ...
    eti_brm <- bayestestR::ci(bfit()[["bayes"]], ci = size) %>% 
      select(Parameter, CI_low, CI_high)
    sigma_eti <- bayestestR::ci(draws$sigma, ci = size) %>% 
      mutate(Parameter = "sigma") %>% 
      select(Parameter, CI_low, CI_high)
    eti_brm <- bind_rows(eti_brm, sigma_eti)
    ...
    result_table <- tibble(
      ...
      !!paste0(size_label, " HDI") := paste0("(", round(hdi_brm$HDI_low, 2),
                                              ", ", round(hdi_brm$HDI_high, 2),
                                              ")"),
      !!paste0(size_label, " ETI") := paste0("(", round(eti_brm$CI_low, 2), ", ",
                                            round(eti_brm$CI_high, 2), ")")
    )
